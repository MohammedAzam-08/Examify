...
const StudentProgressNew: React.FC = () => {
  console.log('StudentProgress component mounted');
  ...
  useEffect(() => {
    const fetchStudentProgress = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        console.log('Loading student progress for ID:', studentId); // Debug log
        ...
        const studentResponse = await fetch(`/api/users/students/${studentId}`, {
        ...
        const submissionsResponse = await fetch(`/api/submissions/student/${studentId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const subjectData = Object.values(subjectPerformance).map((data: { subject: string; totalScore: number; maxScore: number; count: number }) => ({
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-blue-800">
                      {(stats?.improvementTrend ?? 0) >= 0 ? '+' : ''}{(stats?.improvementTrend ?? 0).toFixed(1)}%
                    </p>
                  </div>
                </div>
              </div>
